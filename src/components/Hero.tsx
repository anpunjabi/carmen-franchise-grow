      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col md:flex-row items-center">
          ...
          animationDelay: '0.1s'
            ...
            {isElementVisible(getContentId('hero-heading')) && (
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-carmen-navy mb-6 leading-tight text-balance max-w-md lg:max-w-xl" data-editable-id={getContentId('hero-heading')}>
                <EditableText id={getContentId("hero-heading-1")} as="span">
                  You're losing revenue at the unit level
                </EditableText>
                <EditableText id={getContentId("hero-heading-2")} as="span" className="bg-carmen-gradient bg-clip-text text-transparent">
                  {' shouldn\'t be this hard'}
                </EditableText>
              </h1>
            )}
            {isElementVisible(getContentId('hero-description')) && (
              <EditableText id={getContentId("hero-paragraph")} as="p" className="text-lg text-gray-600 mb-8 max-w-lg text-balance" data-editable-id={getContentId('hero-description')}>
            ...
            {isElementVisible(getContentId('hero-buttons')) && (
              <div className="flex flex-col sm:flex-row gap-4" data-editable-id={getContentId('hero-buttons')}>
                <Button className="bg-carmen-gradient text-white hover:opacity-90 transition-all duration-300 font-medium px-6 py-3 rounded-xl shadow-soft active:scale-95 transform" onClick={() => window.location.href = '#contact'}>
                  <EditableText id={getContentId("hero-button-primary")}>Get Started</EditableText>
                ...
                <Button variant="outline" className="border-carmen-teal text-carmen-teal hover:bg-carmen-teal/10 rounded-xl" onClick={() => window.location.href = '#features'}>
                  <EditableText id={getContentId("hero-button-secondary")}>Explore Features</EditableText>
                </Button>
              </div>
            )}
          </div>
          
          {isElementVisible(getContentId('hero-image-section')) && (
            <div className="md:w-1/2 md:pl-12 fade-in-up" style={{
            animationDelay: '0.3s'
          }} data-editable-id={getContentId('hero-image-section')}>
                ...
                <div className="relative bg-white p-2 rounded-3xl shadow-soft overflow-hidden border border-white">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white/90 to-white/0 p-4">
                    <div className="bg-white/90 backdrop-blur-sm p-4 rounded-2xl shadow-sm border border-white">
                      <EditableText id={getContentId("hero-card-title")} as="h3" className="text-carmen-navy font-semibold mb-1">
                        Custom BPM Dashboard
                      </EditableText>
                      <EditableText id={getContentId("hero-card-description")} as="p" className="text-sm text-gray-600">
